...
class PReNet(nn.Module):
    ...
    def forward(self, input):
        ...
        # if not isinstance(input . torch.tensor(1).cuda()):
        #     input = input.cuda()

        input = torch.unsqueeze(input , 0 )   #add by pl
        batch_size, row, col = input.size(0),  input.size(2), input.size(3)

        # batch_size, row, col = 1, input.size(1), input.size(2)

        x = input
        h = Variable(torch.zeros(1, 32, row, col))
        c = Variable(torch.zeros(1, 32, row, col))
        ...
        # return x
        return torch.squeeze(x) #add by pl
